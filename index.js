...
const degs = ["to left", "to rigth", "to top", "to bottom"];
const randomDegs = Math.floor(Math.random() * degs.length);
...
const randomCssColors = Math.floor(Math.random() * allCssColors.length);
const randomCssColorsSecond = Math.floor(
  Math.random() * allCssColors.length - 1
);
